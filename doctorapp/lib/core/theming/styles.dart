import 'package:doctorapp/core/theming/colors.dart';
import 'package:doctorapp/core/theming/font_weight_helper.dart';
import 'package:flutter/material.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';

class TextStyles {
  static TextStyle font24BlackBold = TextStyle(
    color: Colors.black,
    fontWeight: FontWeight.bold,
    fontSize: 24.sp,
  );
  static TextStyle font24BlueBold = TextStyle(
    color: Colors.blue,
    fontWeight: FontWeightHelper.bold,
    fontSize: 24.sp,
  );
  static TextStyle font32blueBold = TextStyle(
    color: Colors.blue,
    fontWeight: FontWeightHelper.bold,
    fontSize: 32.sp,
  );
  static TextStyle font13GrayReguler = TextStyle(
    color: Colors.grey,
    fontWeight: FontWeightHelper.regular,
    fontSize: 13.sp,
  );
  static TextStyle font16WhiteMedium = TextStyle(
    color: Colors.white,
    fontWeight: FontWeightHelper.medium,
    fontSize: 16.sp,
  );
  static TextStyle font14GrayReguler = TextStyle(
    color: Colors.grey,
    fontWeight: FontWeight.normal,
    fontSize: 14.sp,
  );
  static TextStyle font14LightGreyReguler = TextStyle(
    color: ColorMenager.lightGrey,
    fontWeight: FontWeight.normal,
    fontSize: 14.sp,
  );
  static TextStyle font14DarkBlueMedium = TextStyle(
    color: ColorMenager.darkBlue,
    fontWeight: FontWeight.normal,
    fontSize: 14.sp,
  );
  static TextStyle font13BlueReguler = TextStyle(
    color: ColorMenager.mainBlue,
    fontWeight: FontWeight.normal,
    fontSize: 13.sp,
  );
  static TextStyle font16WhiteSemiBold = TextStyle(
    color: Colors.white,
    fontWeight: FontWeightHelper.semiBold,
    fontSize: 16.sp,
  );
  static TextStyle font18WhiteMedium = TextStyle(
    color: Colors.white,
    fontWeight: FontWeightHelper.medium,
    fontSize: 18.sp,
  );
  static TextStyle font18DarkBlueSemiBold = TextStyle(
    color: ColorMenager.darkBlue,
    fontWeight: FontWeightHelper.semiBold,
    fontSize: 18.sp,
  );
  static TextStyle font13BlueSemiBold = TextStyle(
    color: ColorMenager.mainBlue,
    fontWeight: FontWeightHelper.semiBold,
    fontSize: 13.sp,
  );
  static TextStyle font13DarkBlueMedium = TextStyle(
    color: ColorMenager.darkBlue,
    fontWeight: FontWeight.normal,
    fontSize: 13.sp,
  );
  static TextStyle font13GrayRegular = TextStyle(
    color: ColorMenager.grey,
    fontWeight: FontWeight.normal,
    fontSize: 13.sp,
  );
  static TextStyle font14BlueSemiBold = TextStyle(
    color: ColorMenager.mainBlue,
    fontWeight: FontWeightHelper.semiBold,
    fontSize: 14.sp,
  );
  static TextStyle font15DarkBlueMedium = TextStyle(
    color: ColorMenager.darkBlue,
    fontWeight: FontWeightHelper.medium,
    fontSize: 15.sp,
  );
  static TextStyle font12GrayMedium = TextStyle(
    color: ColorMenager.grey,
    fontWeight: FontWeightHelper.medium,
    fontSize: 12.sp,
  );
  static TextStyle font18DarkBlueBold = TextStyle(
    color: ColorMenager.darkBlue,
    fontWeight: FontWeight.bold,
    fontSize: 18.sp,
  );
  static TextStyle font12GrayReguler = TextStyle(
    color: Colors.grey,
    fontWeight: FontWeightHelper.regular,
    fontSize: 12.sp,
  );
  static TextStyle font12BlueReguler = TextStyle(
    color: Colors.blue,
    fontWeight: FontWeightHelper.regular,
    fontSize: 12.sp,
  );
  static TextStyle Font12DarkBlueReguler = TextStyle(
    color: Colors.grey,
    fontWeight: FontWeightHelper.regular,
    fontSize: 12.sp,
  );
}
